...
import { BRAND, SOURCE_NAME, BOT_USERNAME, CHANNEL_ID } from 'lib/config';
...
/**
 * The hourly channel post. Deliberately more compact than the private-chat card:
 * a channel feed is skimmed, not read, and one of these lands every hour.
 */
export function channelPost(snapshot, today) {
  const s = snapshot.stats;
  const t = snapshot.tehranNow;

  let m = `💵 <b>قیمت تتر</b> — ${esc(t.jalaliLong)} · ساعت ${t.clock}\n`;
  m += `${RULE}\n`;
  m += `🟢 <b>ارزان‌ترین خرید</b>\n     <b>${faNum(s.bestBuy)}</b> تومان — ${esc(s.bestBuyName)}\n\n`;
  m += `🔴 <b>بهترین فروش</b>\n     <b>${faNum(s.bestSell)}</b> تومان — ${esc(s.bestSellName)}\n\n`;
  m += `⚖️ میانگین بازار: <b>${faNum(s.avg)}</b> تومان\n`;
  m += `📐 بازه بازار: ${faNum(s.minAsk)} تا ${faNum(s.maxAsk)}\n`;

  if (today && today.samples > 1) {
    const diff = s.avg - today.open;
    const pct = today.open ? (diff / today.open) * 100 : 0;
    const arrow = diff > 0 ? '🔺' : diff < 0 ? '🔻' : '➖';
    m += `${arrow} از ابتدای امروز: <b>${faNum(Math.abs(diff))}</b> تومان (${faPct(pct)})\n`;
  }

  m += `${RULE}\n`;
  m += `✅ <b>${faNum(s.count)}</b> صرافی معتبر`;
  if (s.rejectedCount) m += ` · 🧹 <b>${faNum(s.rejectedCount)}</b> نرخ خطادار حذف شد`;
  m += `\n\n🤖 قیمت لحظه‌ای و هشدار قیمت: @${BOT_USERNAME}\n📣 ${CHANNEL_ID}`;
  return m;
}
...
export function helpMessage(isAdminUser) {
  ...
  if (isAdminUser) {
    ...
    m += `/channel — ارسال فوری پست کانال (<code>/channel dry</code> برای پیش‌نمایش)\n`;
    m += `/setchannel @نام — تعیین کانال پست خودکار\n`;
    m += `/forcejoin on|off — عضویت اجباری کانال برای پیوی\n`;
